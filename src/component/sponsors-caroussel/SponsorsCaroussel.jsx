...
export default function SponsorsSection({ sponsors = defaultSponsors, loading = false }) {
  ...
  const list = useMemo(() => (Array.isArray(sponsors) && sponsors.length ? sponsors : defaultSponsors), [sponsors]);
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto w-full space-y-8 relative z-10">
        ...
        <div className={`text-center space-y-3 transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <h2 className="text-4xl sm:text-5xl font-bold text-base-content">
            Supporting The <span className="bg-gradient-to-r from-primary via-secondary to-accent bg-clip-text text-transparent animate-gradient">Nexian</span> Mission
          </h2>
        ...
        <div className="flex justify-center items-center">
          <div className="w-full max-w-6xl">
            {loading ? (
              ...
              <div className={`grid gap-6 ${list.length === 1 ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2 md:grid-cols-3"}`}>
                {list.map((s) => (
                  <div
                    key={s.id}
                  ...
                  >
                    <a href={s.lien} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center w-full">
                      ...
                      {s.description && (
                        <p className="mt-2 text-sm text-base-content/60 max-w-[30ch]">{s.description}</p>
                      )}
      ...
      <style jsx>{`
        ...
        .btn.btn-circle:active { transform: translateY(-50%) !important; }
      `}</style>
    </section>
  );
}
